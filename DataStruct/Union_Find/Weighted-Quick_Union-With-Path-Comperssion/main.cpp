/*
*   This is improved algorithm for Quick_Union
*       when two tree(p, q) merging, there is a weight for one add to another(add p to q or add q to p)
*       when find the root( or group number) of the node, there is a path compression
*   Writed by Cong
*   2017.11.11
*/
...
private:
    int *id;
    int *size;                                  //the size of each tree
...
UF::UF(int n = def_num)
{
    ...
    for(count_i = 0; count_i < n; count_i += 1)
    {
        ...
        size[count_i] = 1;                       //the default size of each tree(root) is 1
    }
    num = n;
...
int UF::UFfind(int p)
{
    ...
    {
        //there will compress the path from the node to the root
        //change the parent node of this node to it's grandparent node
        id[p_temp] = id[id[p_temp]];
...
void UF::UFunion(int p, int q)
{
    ...
    if(p_id != q_id)
    {
        //the weight is the size of each tree(group)
        if(size[q_id] <= size[p_id])
        {
            id[q_id] = p;
            size[p_id] += size[q_id];
        }
